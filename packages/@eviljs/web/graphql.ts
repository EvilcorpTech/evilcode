import {safeObject} from '@eviljs/std/type-safe.js'
...
export function sendGraphqlQuery<T = unknown>(fetch: Fetch, query: string): Promise<T> {
    const method = HttpMethod.Post
    const url = asBaseUrl(fetch.baseUrl)
    const conf = withRequestGraphql(query)

    return fetch.request(method, url, conf)
        .then(it => it.json() as Promise<unknown>)
        .then(it => safeObject(it).data as T)
}
