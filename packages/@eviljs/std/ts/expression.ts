...
*/
export function lookup(ctx: CtxWithResolver, pathExp: Exp<Ctx, string>): unknown {
    const {$, resolvers} = ctx
    assertObject($)
...
*/
export function evaluateTree(operators: Record<PropertyKey, Operator>, tree: Tree, ctx: Ctx): unknown {
